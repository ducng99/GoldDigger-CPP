...
bool Sprite::Initialise(Texture & texture, int x, int y, int width, int height)
{
	m_pTexture = &texture;

	if (width <= m_pTexture->GetWidth() && height <= m_pTexture->GetHeight() && x >= 0 && x <= m_pTexture->GetWidth() && y >= 0 && y <= m_pTexture->GetHeight())
	{
		m_textureX = x;
		m_textureY = y;
		m_width = width;
...
int
Sprite::GetTextureX() const
{
	return (m_textureX);
}

int
Sprite::GetTextureY() const
{
	return (m_textureY);
}
